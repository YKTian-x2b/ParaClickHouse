...
#ifdef ENABLE_CUDA
        AggregateFunctionPtr kai_res = found.creator(name, argument_types, parameters, settings);

        if(name.starts_with("sum")) {
            const auto &t_typeId = argument_types[0]->getTypeId();
            size_t aggregateFunction_T_Size = 0;

#define KAI_T_DISPATCH(TYPE) \
            if (t_typeId == TypeIndex::TYPE){ \
                aggregateFunction_T_Size = sizeof(TYPE); \
                std::cout << "KAI_T_DISPATCH: " << aggregateFunction_T_Size << std::endl; \
            }
            FOR_BASIC_NUMERIC_TYPES(KAI_T_DISPATCH)
#undef KAI_T_DISPATCH

            const auto &tResult_typeId = kai_res->getReturnType()->getTypeId();
            size_t aggregateFunction_TResult_Size = 0;
            
#define KAI_TRESULT_DISPATCH(TYPE) \
            if (tResult_typeId == TypeIndex::TYPE){ \
                aggregateFunction_TResult_Size = sizeof(TYPE); \
                std::cout << "KAI_TRESULT_DISPATCH: " << aggregateFunction_TResult_Size << std::endl; \
            }
            FOR_BASIC_NUMERIC_TYPES(KAI_TRESULT_DISPATCH)
#undef KAI_TRESULT_DISPATCH

            ResourceManagementOnce::instance().initResource(aggregateFunction_T_Size, aggregateFunction_TResult_Size, settings->max_block_size);
        }
        return kai_res;
#else
